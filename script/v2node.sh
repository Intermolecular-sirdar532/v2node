...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/Mcloud136/v2node/master/script/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/Mcloud136/v2node/master/script/install.sh) $version
    if [[ $? == 0 ]]; then
